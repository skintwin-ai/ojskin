...
class TemplateManager extends PKPTemplateManager {
	...
	function setupBackendPage() {
		...
		// Add agents management link (SKZ Agents Plugin)
		$plugin = PluginRegistry::getPlugin('generic', 'skzAgents');
		if ($plugin && $plugin->getEnabled($request->getContext()->getId()) && array_intersect([ROLE_ID_SITE_ADMIN, ROLE_ID_MANAGER], $userRoles)) {
			$agentsLink = [
				'name' => __('plugins.generic.skzAgents.management.title'),
				'url' => $router->url($request, null, 'management', 'settings', 'agents'),
				'isCurrent' => $request->getRequestedPage() === 'management' && 
							   $request->getRequestedOp() === 'settings' && 
							   (isset($request->getRequestedArgs()[0]) && $request->getRequestedArgs()[0] === 'agents'),
			];

			$index = array_search('settings', array_keys($menu));
			if ($index === false || count($menu) === $index) {
				$menu['agents'] = $agentsLink;
			} else {
				$menu = array_slice($menu, 0, $index, true) +
						['agents' => $agentsLink] +
						array_slice($menu, $index, null, true);
