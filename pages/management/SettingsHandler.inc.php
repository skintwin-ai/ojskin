...
class SettingsHandler extends ManagementHandler {
	...
	/**
	 * Display agent management settings
	 * @param $args array
	...
	function agents($args, $request) {
		...
		if (!$plugin || !$plugin->getEnabled($context->getId())) {
			...
			$templateMgr->assign([
				'pageTitle' => __('plugins.generic.skzAgents.management.title'),
				'pluginEnabled' => false,
				'errorMessage' => __('plugins.generic.skzAgents.management.notEnabled')
			]);
		} else {
			// Plugin is enabled, set up agent management interface
			$templateMgr->assign([
				'pageTitle' => __('plugins.generic.skzAgents.management.title'),
				'pluginEnabled' => true,
			]);

			// Get agent status and configuration from the plugin
			try {
				$agentStatus = $this->getAgentStatus($plugin, $context);
				$templateMgr->assign('agentStatus', $agentStatus);
			} catch (Exception $e) {
				$templateMgr->assign('errorMessage', __('plugins.generic.skzAgents.management.connectionError'));
			}

			// Set up API URLs for AJAX operations
			$baseUrl = $dispatcher->url($request, ROUTE_COMPONENT, $context->getPath(), 
				'plugins.generic.skzAgents.controllers.SKZAgentManagementHandler');
			
			$templateMgr->assign([
				'apiBaseUrl' => $baseUrl,
				'startAgentUrl' => $baseUrl . '/startAgent',
				'stopAgentUrl' => $baseUrl . '/stopAgent',
				'getStatusUrl' => $baseUrl . '/getStatus',
				'configureUrl' => $baseUrl . '/configure',
			]);
		}

		$templateMgr->display('management/agents.tpl');
	}

	/**
	 * Get status of all agents from the SKZ framework
	 * @param $plugin SKZAgentsPlugin
	 * @param $context Context
	 * @return array Agent status information
	 */
	private function getAgentStatus($plugin, $context) {
		// Define the 7 SKZ autonomous agents
		$agents = [
			'research_discovery' => __('plugins.generic.skzAgents.agent.researchDiscovery'),
			'submission_assistant' => __('plugins.generic.skzAgents.agent.submissionAssistant'),
			'editorial_orchestration' => __('plugins.generic.skzAgents.agent.editorialOrchestration'),
			'peer_review' => __('plugins.generic.skzAgents.agent.peerReview'),
			'quality_assurance' => __('plugins.generic.skzAgents.agent.qualityAssurance'),
			'publication_formatting' => __('plugins.generic.skzAgents.agent.publicationFormatting'),
			'workflow_orchestration' => __('plugins.generic.skzAgents.agent.workflowOrchestration'),
		];
		...
		foreach ($agents as $agentId => $agentName) {
			$status[$agentId] = [
				'name' => $agentName,
				'id' => $agentId,
				'status' => 'unknown', // Will be updated via AJAX
				'lastActivity' => null,
				'processedTasks' => 0,
				'configuration' => []
			];
